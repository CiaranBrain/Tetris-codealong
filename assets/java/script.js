...
// move the tetromino right, unless is at the endge or there is a blockage 
...
function rotate() {
    undraw()
    currentRotation++
    if (currentRotation === current.length) { // if the current rotation gets to 4, make it got back to 0
        currentRotation = 0
    }
    current = theTestrominoes[random][currentRotation]
    draw()
...
const upNextTetrominoes = [
    ...
    [1, displayWidth + 1, displayWidth + 1, displayWidth + 2],//tTerromino
...
startBtn.addEventListener('click', () => {
    if (timerId) {
        ...
        draw()
        timerId = setInterval(moveDown, 1000)
        nextRandom =Math.floor(Math.random() * theTestrominoes.length)
        displayShape()
    }
})